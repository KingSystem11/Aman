        ...
        const createTrackListPage = async (page = 1) => {
            ...
            currentPageTracks.forEach((track, index) => {
                if (!track) {
                    ...
                    description += `**${start + index + 1}.** [${title.length > 45 ? title.slice(0, 42) + '…' : title}](${uri}) \`${formatDuration(length)}\`\n`;
                }
            });

            const embed = new EmbedBuilder()
                .setColor(0x5865F2)
                .setTitle(`${emojis.music} ${playlistName}`)
                .setDescription(description)
                .setFooter({ text: `Page ${page}/${totalPages} • ${playlist.tracks.length} tracks in playlist` });
