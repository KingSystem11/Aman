...
async function _importFromSpotify(message, url) {
    ...
    const loadingEmbed = new EmbedBuilder()
        .setColor(0x5865F2)
        .setDescription(`${emojis.music} Loading Spotify playlist...`);
    ...
    if (existingPlaylist) {
        const embed = new EmbedBuilder()
            .setColor(0x5865F2)
            .setTitle(`${emojis.music} Playlist Already Exists`)
            .setDescription(`You already have a playlist named **${spotifyPlaylistName}**. What would you like to do?`);
        ...
        const reply = await loadingMsg.edit({ embeds: [embed], components: [row] });
        ...
        collector.on('end', async (collected, reason) => {
            if (reason === 'time') {
                ...
                loadingMsg.edit({ embeds: [timeoutEmbed], components: [] });
            }
        });
    } else {
        const playlistCount = await Playlist.count({ where: { userId } });
